...
export const treasuryService = {
  ...
  createBankAccount: async (organizationId: string, data: CreateBankAccountInput) => {
    ...
    const bankAssetAccount = accounts.find((a: any) => a.name === "Bank Account" || a.code === "1030");
    const parentId = bankAssetAccount ? bankAssetAccount.id : undefined;
    ...
    const { currentBalance, ...bankAccountData } = data;

    // 2. Create the Bank Account linked to the new GL Account
    const bankAccount = await treasuryRepository.createBankAccount(organizationId, glAccount.id, bankAccountData);
    ...
    if (currentBalance && Number(currentBalance) > 0) {
      ...
      const lines = [
        ...
        },
        {
          ...
          debit: 0,
          credit: amount,
          description: "Initial Balance Offset",
        }
      ];
  ...
  updateBankAccount: async (organizationId: string, id: string, data: UpdateBankAccountInput) => {
    ...
    return treasuryRepository.updateBankAccount(organizationId, id, data);
  },
