...
jest.mock("../../../src/config/database.js", () => {
  ...
  return {
    ...
    default: {
      ...
      role: { create: jest.fn(), delete: jest.fn() },
...
describe("roleService", () => {
  ...
  describe("listRoles", () => {
    it("should list and serialize roles", async () => {
      ...
      expect(roleRepository.listRoles).toHaveBeenCalledWith("o1");
  ...
  describe("createRole", () => {
    it("should throw 400 if role name is invalid", async () => {
      ...
      await expect(roleService.createRole("o1", { name: "!", permissionNames: [] })).rejects.toThrow(ApiError);
    });

    it("should throw 400 if trying to create a system role", async () => {
      (slugify as jest.Mock).mockReturnValue("admin");
      await expect(roleService.createRole("o1", { name: "Admin", permissionNames: [] })).rejects.toThrow(ApiError);
    });
    ...
    it("should throw 400 if permission not found", async () => {
      ...
      await expect(roleService.createRole("o1", { name: "custom", permissionNames: ["p"] })).rejects.toThrow(ApiError);
    });

    it("should create role and map permissions", async () => {
      (slugify as jest.Mock).mockReturnValue("custom");
      (prisma.permission.findMany as jest.Mock).mockResolvedValue([{ id: "p1" }]);
      (prisma.role.create as jest.Mock).mockResolvedValue({ id: "r1", rolePermissions: [] });

      await roleService.createRole("o1", { name: "Custom", permissionNames: ["p"] });
  ...
  describe("updateRole", () => {
    it("should throw 404 if role not found", async () => {
      ...
      await expect(roleService.updateRole("o1", "r1", {})).rejects.toThrow(ApiError);
    });
    ...
    it("should update role using transaction", async () => {
      ...
      await roleService.updateRole("o1", "r1", { permissionNames: ["p"] });
  ...
  describe("deleteRole", () => {
    it("should throw 404 if role not found", async () => {
      (roleRepository.findRoleById as jest.Mock).mockResolvedValue(null);
      await expect(roleService.deleteRole("o1", "r1")).rejects.toThrow(ApiError);
    });
    ...
    it("should delete role", async () => {
      ...
      await roleService.deleteRole("o1", "r1");
